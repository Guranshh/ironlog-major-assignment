...
export async function BlogDetail({ post }: { post: Post & { liked: boolean } }) {
  ...
  return (
    <>
      <article data-test-id={`blog-post-${post.id}`} className="flex flex-col gap-5 py-8">
        <img
          src={post.imageUrl}
        ...
        <div className="text-secondary flex flex-wrap items-center gap-x-3 gap-y-1 text-xs">
          <Link href={`/category/${toUrlPath(post.category)}`} className="text-wsu font-semibold">
            {post.category}
          </Link>
          <span>{date}</span>
        ...
        <Link href={`/post/${post.urlId}`} className="text-primary text-4xl leading-tight font-bold">
          {post.title}
        </Link>
      ...
      <Comments postId={post.id} urlId={post.urlId} />
    </>
  );
}